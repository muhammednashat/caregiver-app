...
open abstract class AuthBaseFragment<T : ViewDataBinding> : BaseFragment2<T>() {
    ...
    suspend fun login(callBack: (Boolean) -> Unit) {

        FirebaseService.login(_viewModel.email.value?.trim()!!, _viewModel.password.value?.trim()!!,requireActivity(),sharedPreferences) {
